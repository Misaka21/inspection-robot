...
注意：以上是“功能分组”，不是“端口分组”。通信时必须按命令号走固定端口。
...
- `查询机器人急停状态.md`（1012）
- `查询机器人报警状态.md`（1050）
- `配置机器人推送端口_4091.md`（4091，配置 API）
- `配置机器人推送端口.md`（9300，推送 API）
- `机器人推送.md`（19301）
...
| 端口 | API 类别 | 命令号 |
| --- | --- | --- |
| `19204` | 机器人状态 | `1004,1005,1006,1007,1012,1020,1021,1022,1050,1060,1110` |
| `19205` | 机器人控制 | `2000,2002,2004,2010,2022` |
| `19206` | 机器人导航 | `3001,3002,3003,3051,3066,3067` |
| `19207` | 机器人配置 | `4005,4006,4009,4091` |
| `19210` | 其他 API | `6004` |
| `19301` | 推送 API | `9300`（部分版本用于配置推送），`19301`（机器人主动推送消息类型） |

实现规则：

- 先按命令号选端口，再发包。
- 不允许“为省连接数”把不同类别命令混发到同一端口。
- `19301` 是推送通道，建议独立连接管理。
- 推送配置命令存在 `4091@19207` 与 `9300@19301` 两种入口时，优先按机器人版本文档执行。
