...
def _yaw_from_quaternion(x: float, y: float, z: float, w: float) -> float:
    # ROS uses ENU; yaw extraction matches tf2 yaw from quaternion.
...
def ros_agv_status_to_pb(pb2, ros_msg) -> object:
    pose2d = pb2.Pose2D(
...
def ros_arm_status_to_pb(pb2, ros_msg) -> object:
    return pb2.ArmStatus(
        connected=bool(ros_msg.connected),
        arrived=bool(ros_msg.arrived),
        moving=bool(ros_msg.moving),
...
def ros_system_state_to_task_status(pb2, ros_state, ctx: GatewayTaskContext) -> object:
    task_id = str(ros_state.task_id) if getattr(ros_state, "task_id", "") else ctx.task_id

    # Interlock inference (UI hint): allow arm action only when AGV is ready.
    interlock_ok = bool(
    ...
    return pb2.TaskStatus(
        task_id=task_id,
        phase=pb2.TaskPhase.Value("TASK_PHASE_UNSPECIFIED")
        if int(ros_state.phase) <= 0
        else int(ros_state.phase),
        ...
        agv=ros_agv_status_to_pb(pb2, ros_state.agv),
        arm=ros_arm_status_to_pb(pb2, ros_state.arm),
...
def ros_nav_map_to_pb(pb2, map_resp, media_ref: Optional[object]) -> object:
